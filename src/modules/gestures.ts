...
const wheelHandler = (e: WheelEvent) => {
    dispatchGesture(e.target as Element, {name: "wheel", x: e.clientX, y: e.clientY, event: e, type:"mouse"});
...
const touchendHandler = (e: TouchEvent) => {
    ...
    if (touch.isDragging) {
        ...
        // tap detection
        if (touch.consecutiveTaps === 0) dispatchGesture(touch.activeElement, {name: "tap", x: touch.x, y: touch.y, type:"touch"});

        // double-tap detection
        touch.consecutiveTaps++;
        window.setTimeout(() => {
            if (touch.consecutiveTaps > 1) dispatchGesture(touch.activeElement, {name: "double-tap", x: touch.x, y: touch.y, type:"touch"});
